...
/**
 * Displays validation errors and warnings with proper accessibility
 * and visual distinction between error and warning states
 */
export function ValidationErrors({ errors, warnings = [] }: ValidationErrorsProps) {
  ...
  return (
    <div className="space-y-2" role="alert" aria-live="polite">
      ...
      {warnings.length > 0 && (
        <div
          className="bg-yellow-500/10 border border-yellow-500/30 rounded-md p-3"
          role="status"
          aria-label={`${warnings.length} validation warning${warnings.length > 1 ? 's' : ''}`}
        >
          <div className="flex items-start gap-2">
            <AlertTriangle
              className="h-4 w-4 text-yellow-600 dark:text-yellow-500 mt-0.5 flex-shrink-0"
              aria-hidden="true"
            />
            <div className="flex-1">
